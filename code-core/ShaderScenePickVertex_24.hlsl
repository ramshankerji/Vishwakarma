...
/* PARKED, alongside ShaderSceneVertex_24.hlsl: the pick shader for the 24-byte per-vertex-color
vertex. The engine picks with ShaderScenePickVertex_16.hlsl today; this file is kept compiling so the
per-vertex-color variants under "Vertex format variants" in website/software/graphics.md have a
maintained starting point. Nothing includes its generated header yet.

GPU picking vertex shader. Identical transform to ShaderSceneVertex_24, but instead of forwarding
...
// Mirrors ShaderSceneVertex_24.hlsl (and InstanceRecord in RenderScene3D.h) so the pick pass
...
// Mirrors IsVisibleInSubTab in ShaderSceneVertex_24.hlsl - see there for the bit convention. The pick
