...
/* Benchmark harness for the Page2D paging work (id.md §11.4, step 2a). Appends `count` lines to
the active Page2D on a deterministic 10 CU grid - deterministic because step 2c's acceptance is a
frame compared against the pre-change build. The whole burst goes onto the copy queue under ONE
