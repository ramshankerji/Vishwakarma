...
The hidden records stay resident until compaction packs them out.
...
/* One frame's worth of "what the armed 2D tool would commit if the user clicked right now",
published by the engineering thread and read by every render thread showing this Page2D. Same
one-direction, one-frame-stale contract as SnapHoverState: a CLICK never reads this back - the
click re-resolves from its own pixel, so a stale preview can never become a stale object.

Engineering records rather than a parametric description or ready-made GPU records, deliberately.
A preview is only worth drawing if it is what lands, so it is BUILT by the same Build*Record code
the click path uses and CONVERTED by the same ToGpu* / Append* converters the copy thread uses.
Neither half owns a second copy of the geometry that could drift from the first.

Guarded by TabCad2DStorage::previewMutex, which the render thread holds while it converts straight
into its upload buffer - so a large selection's preview is never deep-copied per window per frame. */
struct Cad2DPreviewContent {
    int32_t subTabSlot = -1;      // The view whose cursor produced it; other views must not draw it.
    ...
    // clear(), never shrink: this is rewritten at input rate and reuses its capacity every time.
    void Clear() {
        subTabSlot = -1;
        containerMemoryId = 0;
        lines.clear();
    ...
    bool Empty() const {
        return lines.empty() && polylines.empty() && polygons.empty() && circles.empty() &&
            ellipses.empty() && arcs.empty() && texts.empty();
    }
    // Publishing is a swap, not a copy: the engineering thread hands over the buffers it just
    // filled and takes back the ones the render threads are done with, so a publish every mouse
    // move allocates nothing after the first.
    void Swap(Cad2DPreviewContent& other) {
        std::swap(subTabSlot, other.subTabSlot);
        std::swap(containerMemoryId, other.containerMemoryId);
        lines.swap(other.lines);
        polylines.swap(other.polylines);
        polygons.swap(other.polygons);
        circles.swap(other.circles);
        ellipses.swap(other.ellipses);
...
/* Resolves the snap under `input` ONCE and publishes both things that depend on it: the hover
marker (snapping.md sections 8 and 12) and the in-progress creation / transform preview
(2Drendering.md, "Live creation preview"). One resolve, because the candidate scan is the expensive
part and the two publications want the identical answer - a preview drawn to a different point than
the marker promises would be worse than no preview. Called ONCE per drain of the tab's input queue,
from the coalesced snapHoverInput - never per mouse report. No-op unless the input view is a Page2D. */
...
/* Benchmark harness for the Page2D paging work. Appends `count` lines to the active Page2D on a
deterministic 10 CU grid - deterministic so a frame can be compared against an earlier build. The
whole burst goes onto the copy queue under ONE lock so it arrives as one batch; enqueuing line by
line would let the copy thread drain partial batches and rebuild the growing sheet dozens of times
over. */
void Cad2DGenerateBulkLines(DATASETTAB& tab, uint32_t count);
/* The other half of that harness, for compaction. Moves the last `count` lines of the active
Page2D up by half a CU - `count` DISTINCT objects modified once each, which is what makes holes:
each one appends a new run and hides its old. `count` 0 means every line of the container. Pressing it
