...
All vertexes are positioned on object local space. World matrix applied in vertex shader.
This enables, moving even 1000vertex objects just a 48 bytes world matrix update per oject.
We use packed 48 bytes world matrix instead of 64 bytes to save bandwidth.
Since last row is always 0,0,0,1, we can omit it. In shader, we reconstruct the last row.

Separate render threads (1 per monitor) and single Copy thread. Copy thread is the ringmaster of VRAM!
Separate render threads per monitor are in VSync with monitors unique refresh rate.
Here separate render queue per monitor.

We use ExecuteIndirect command with start vertex location instead of DrawIndexedInstanced per object.
I want per tab VRAM isolation, each tab will be completely separate.
Except for uncloseble tab 0 which stores common textures and UI elements.

To support 100s of simultaneous tab, we start with small heap say 4MB per tab and grow heap size only when necessary.
Each page could be a mixture of various geometry types. Say Cylinders, Cubes, I beams etc.
Instead of allocating 1 giant 256MB buffer. Don't manually destroy heaps on tab switch. Use Evict. 
It allows the OS to handle the caching. 
If the user clicks back to a heavy tab, MakeResident is faster than re-creating heaps. Tab 0 is always resident. 
Eviction happens with a time lag of few seconds. 
Advanced system memory budget based eviction strategy after rest of spec implemented.

Each page will be accompanied by a corresponding ExecuteIndirect argument buffer.
...
Copy thread than update the next double buffer and record the hole in Vertex/index buffer.
Except for currently filling head buffer,

Maintain a Free-List Allocator (e.g., a Segregated Free List) on the CPU. Per Tab.
The Allocator knows: "I have a 12KB middle gap in Page 3, and a 40KB middle gap in Page 8."
When a 10KB request comes in, the Allocator immediately returns "Page 3". No iterating through Page objects.
If freelist says none of existing pages can accommodate new geometry, than create new heap/placed resource buffer.
Free list does not track internal holes created from deleting objects. 
Only middle empty space. Aggregate holes are tracked per page. Defragmented occasionally.
...
• ​Topology: Triangles (Solid) vs Lines (Wireframe) (PSO Requirement). 
    We cannot draw lines and triangles in the same call.
• ​Culling: Single-Sided vs Double-Sided (PSO Requirement) . Sheet metal vs Solids.
    Since section is a common use case, perhaps we could have all geometry double sided. To be ascertained latter.
• ​Buffer Pages (N): How many 256MB blocks you are using.
​Total Unique Batches = 2 x 2 x 2 x 2 x N = 16 x N
This will ensure no pipeline state reset while rendering single Page. ExecuteIndirect call for every Page.

To be clarified latter: How do we  handle repeat geometry ? Say bolts.
They will only need set of vertex/index buffers. We can draw them with different world matrices.
...
​X: 10 bits (0 to 1023), ​Y: 10 bits (0 to 1023), ​Z: 10 bits (0 to 1023), ​Padding: 2 bits (unused)
​Total: 32 bits (4 Bytes). Why this is perfect for Normals:
​Size: It is 3x smaller than 12-byte normal. (4 bytes vs 12 bytes). ​Precision: 10 bits gives us 2^{10} = 1024 steps. 
Since normals are always between -1.0 and 1.0, this gives you a precision of roughly 0.002.
...
​Halves the Allocation Overhead: We only manage 1 heap/resource per 4MB page instead of 2.
...
Resource state is together. i.e. D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER | D3D12_RESOURCE_STATE_INDEX_BUFFER
...
New geometry is appended (in the middle ) only if both new vertex and index buffers fit inside.
Otherwise allocate new buffer. Copy thread also does batching. 
It aggregates all(who fit in  current buffer) objects coming from engineering thread into single GPU upload.
The Copy Thread should consume batches of updates, 
coalescing them into single ExecuteCommandList calls where possible to reduce API overhead.

"Big Buffer" fallback. If Allocation_Size > Max_Page_Size, 
allocate a dedicated Committed Resource just for that object, bypassing the paging system.
Handles large STL. or terrain map. Treat "Big Buffers" as a special Page Type. Add a "Large Object List" to your loop.
Do not try to jam them into the standard EI logic if they require unique resource bindings per object.
1 separate draw command for such Jumbo objects.
...
Copy queue picks it up , once defragmented, return the new resource.
I am willing to accept the freeze of few frames on screen.
This is a recognised engineering  tradeoff. Acceptable to CAD users.

EI Argument Buffers tightly coupled to the Memory Pages. 
When we defragment a Page, we must simultaneously rebuild its corresponding Argument Buffer.
...
No thread stall. Transition that page to copy destination. Copy new data. 
Transition back to render status for render thread to pick up.
...
Transparency sorting. accepting imperfect sorting for "Glass" pages during rotation, 
    and doing a CPU Sort + Args Rebuild only when the camera stops.
Hot page for object drag / active mutation.
Evict logic.
Comput shader frustum culling.
Telemetry. Per-tab VRAM usage graphs. Page fragmentation heatmap. Eviction frequency counters.
    Copy queue stall tracking.
Selection Highlighter methodology.
Mesh Shader on supported hardware (RTX2000 onwards, RX6000 onwards).
Instanced based LOD optimization . Optionally using compute shader.

Miscellaneous Specification: 
There will be a uniform object ID ( 64 bit ) unique across all objects across entire process memory. 
Each object can have up-to 16? different simultaneous variations of vertex geometry / graphics representation.
We am expecting 1000 to 5000 draw calls per frame ?
How should I handle multiple partially overlapping windows? 
Each windows can be independently resized or maximized / minimized.
Lowest distance between object and ALL the different view camera position shall be used by logic threads,
    to decided the Level of Detail.
It will have some mechanism to manage memory over pressure.
To signal the logic threads to reduce the level of detail within some distance.
...
1 Monitor is connected and active to each of these 3 devices.
We can use exactly 1 device for rendering for all monitor!
...
TO-DO LIST : As things get completed, 
    they will be removed from this pending list and get incorporated appropriately in design document.
...
[ ] Transparency Sorting. (CPU Sort + Args Rebuild when camera stops moving).
...
[ ] Asynchronous Resource Creation. (Use D3D12's async resource creation to further reduce stalls
  during heap growth or defragmentation).
[ ] Page Level optimization : Static pages → single draw, Semi-dynamic pages → EI , 
  Highly dynamic pages → EI + GPU compaction
