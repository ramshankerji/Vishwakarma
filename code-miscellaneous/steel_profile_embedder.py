...
"""Generate embedded C++ data for the section catalog (Catalog/profiles_hot_*.csv
plus the parametric family in Catalog/profiles_parametric.csv).
...
# profiles_hot_<FAMILY>.csv -> C++ SteelProfileFamily enumerator. "PARAMETRIC" is
# the one non-hot-rolled family and lives in profiles_parametric.csv: its rows are
# rendered from the member's user parameters, the CSV holding only the defaults,
# with the `series` column (RECT/CIRC/OCT/HEX) picking the outline like BAR does.
...
PARAMETRIC_FILE = "profiles_parametric.csv"

# Parametric series -> default-dimension columns that must be strictly positive.
PARAMETRIC_REQUIRED_BY_SERIES = {
    "RECT": ("h", "b"),
    "CIRC": ("d",),
    "OCT": ("a",),
    "HEX": ("a",),
...
REQUIRED_COLUMNS = {
    ...
    "CHS": ("d", "t"),
    "I": ("h", "b", "tw", "tf"),
    "PARAMETRIC": (),  # Per-series requirement, PARAMETRIC_REQUIRED_BY_SERIES.
...
def parse_csv(path: Path, family_suffix: str) -> list[dict]:
    ...
    with path.open(newline="", encoding="utf-8-sig") as csv_file:
        ...
        for line_number, row in enumerate(reader, start=2):
            ...
            series_required: tuple = ()
            if family_suffix == "PARAMETRIC":
                series = record["series"].upper()
                if series not in PARAMETRIC_REQUIRED_BY_SERIES:
                    raise RuntimeError(f"{context}: parametric series '{record['series']}' "
                                       f"must be one of {sorted(PARAMETRIC_REQUIRED_BY_SERIES)}")
                series_required = PARAMETRIC_REQUIRED_BY_SERIES[series]
            for column, field in geometry.items():
                column_required = (column in required or (flat_bar and column == "b")
                                   or column in series_required)
...
def generate_header(rows: list[dict]) -> str:
    lines = [
        ...
        "// Edit Catalog/profiles_hot_*.csv / profiles_parametric.csv (through",
        "// Catalog/catalog_editor_v2.py), then rebuild.",
...
def main() -> int:
    ...
    try:
        ...
        for suffix in sorted(FAMILY_BY_FILE_SUFFIX):
            file_name = PARAMETRIC_FILE if suffix == "PARAMETRIC" else f"profiles_hot_{suffix}.csv"
            csv_path = catalog_dir / file_name
