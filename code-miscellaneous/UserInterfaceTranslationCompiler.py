...
def validate_short_word_names(canonical_rows: Dict[int, dict], source_path: Path) -> None:
    seen_names = set()

    for world_id, row in canonical_rows.items():
        name = (row.get("ShortWordName") or "").strip()

        if not name:
            print(
                f"ERROR: WorldID {world_id} has empty ShortWordName.",
                file=sys.stderr,
            )
            sys.exit(1)

        if not CPP_IDENTIFIER.match(name):
            print(
                f"ERROR: ShortWordName '{name}' for WorldID {world_id} "
                f"is not a valid C++ identifier.",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if name in seen_names:
            print(
                f"ERROR: Duplicate ShortWordName '{name}'.",
                file=sys.stderr,
            )
            sys.exit(1)
...
def build_translation_data(source_dir: Path) -> List[Tuple[int, List[str]]]:
    ...
    validate_short_word_names(canonical_rows, canonical_path)
...
def build_enum_entries(canonical_rows: Dict[int, dict]) -> List[str]:
    entries = []

    for world_id in sorted(canonical_rows.keys()):
        row = canonical_rows[world_id]
        short_name = row["ShortWordName"].strip()
        entries.append(f"    {short_name} = {world_id},")

    return entries

def write_compiled_header(output_dir: Path, canonical_rows: Dict[int, dict],) -> None:
    enum_entries = "\n".join(build_enum_entries(canonical_rows))

    h_content = f"""// Auto-generated by UserInterfaceTranslationCompiler.py - DO NOT EDIT MANUALLY
// Copyright (c) 2026-Present : Ram Shanker: All rights reserved.
#pragma once
#include <cstdint>
...
def main() -> None:
    ...
    canonical_path = script_dir / "UserInterfaceTranslation.csv"
    canonical_rows = read_csv_by_world_id(canonical_path)
    validate_short_word_names(canonical_rows, canonical_path)

    translation_data = build_translation_data(script_dir)
    write_compiled_header(output_dir, canonical_rows,)
