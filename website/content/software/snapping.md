...
**Watch the units.** Page2D ComputerUnits are millimetres (`2Drendering.md`, "Conventions that stuck"); 3D geometry is SI metres (`LINE_MEMBER` comment). The same `niceRound125` helper is shared but the two worlds land on different absolute steps, and a nominal "1 unit" grid means 1 mm in one and 1 m in the other. Any shared constant expressed in "units" is a bug waiting to happen.
