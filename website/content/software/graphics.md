...
- **Retiring the persistent per-page indirect buffers** — the sharpest remaining VRAM item, since every 4 MB page still reserves a fixed 65,536 × 24 B = 1.5 MB argument buffer (~2.4 GB at 10M objects). It is the largest piece left: the templates become an SRV-only, right-sized `DrawTemplateBuffer`, which in turn forces the GPU pick pass *and* the print path off the 24-byte per-page `ExecuteIndirect` they still execute, and needs the page directory so the shader can expand a `sourcePage` reference into buffer views.
