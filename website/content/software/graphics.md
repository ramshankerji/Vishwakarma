...
- **Instanced rendering is impossible in this form.** Two identical bolts at different places produce different vertex bytes, so no geometry can ever be shared. Phase 5 lists instancing as the single biggest VRAM lever for plant models; it is blocked on generating geometry in a canonical local frame, not on the instancing code.
...
- **The bit index is the sub-tab slot**, passed as a root constant (`b2`) in its own range rather than widened into `b1` — `b1` is what the command signature rewrites per command, so a render-thread value sharing that range would be overwritten by `ExecuteIndirect`. `MV_MAX_SUBTABS` is 128 against 64 mask-addressable SubTabs, so slots past 63 pass a sentinel that disables the test and show everything; the documented remedy for ever needing more is a second mask word, not a change here.
...
- Deliberately **not** built: the render-target rectangle and update scheduling fields named in item 2. The rectangle is still derived per frame by the compositor and scheduling does not exist yet — those belong with Step 10's Viewport classes, and adding empty fields now would only invite them to drift out of date.
...
*As implemented (vertical slice — the GPU compaction path, `ShaderSceneCull.hlsl`):* the first cut keeps **one `ExecuteIndirect` per page** rather than one per Viewport, and regenerates the compacted commands every frame. That single decision collapses most of the scope above: with per-page draws retained, the compute pass reads and writes only through **root descriptors** (templates SRV, mask SRV, output UAV, count UAV), so the shader-visible descriptor heap, the global allocator, the `SceneEpoch` directory, the `(epoch, filter revision, visibility revision)` cull cache and the retirement of the per-page indirect buffers are all deferred without blocking a working GPU cull. The per-page `indirectBuffer` — already rebuilt on every clone — *is* the `DrawTemplateBuffer` for now. The path is behind a default-off toggle (`gUseComputeCull`, debug key `k`).

**Parity is confirmed, and the default deliberately stays off.** The two paths have been run side by side and draw the same scene. The legacy direct-`ExecuteIndirect`-of-all-templates path is therefore not a "fallback until verified" any more — it is a second maintained path, kept because the compute path buys nothing until the deferred work below lands (per-page draws still cost one `ExecuteIndirect` each either way, and the per-object mask test the compute pass moves upstream is cheap while scenes are small). Promoting it to primary is a decision to take when one of the deferred items makes it actually cheaper, not a formality. Until then, treat any change to the draw loop as a change to **both** paths.

**What "compaction" moves here — commands, not geometry.** This is the point most likely to be misread, because the word also names the geometry defragmentation in *Defragmentation logic* / Step 8. The two are unrelated:

- **GPU draw-command compaction (this step, render thread).** Compacts the array of 24-byte `IndirectCommand` records — the draw *templates*. Vertex and index bytes are never read, copied or moved; the geometry pages stay exactly where they are. There is **no temporary allocation**: the output lands in a **persistent per-monitor scratch** (`SceneCullScratch`: a `MV_MAX` 65,536-command `visibleIndirect` buffer plus a 4-byte `visibleCount`), created once when the render thread starts and reused for every page of every frame. Per page the render thread resets the count with a 4-byte `CopyBufferRegion` from a shared zero buffer, dispatches one thread per template, barriers the scratch `UNORDERED_ACCESS → INDIRECT_ARGUMENT`, and issues one `ExecuteIndirect` whose command count comes from `visibleCount`. The same scratch is reused for the next page (the barrier back to `UNORDERED_ACCESS` drains the previous page's draw first).
- **Page compaction (Step 8 / *Defragmentation logic*, copy thread).** *Does* allocate — a fresh 4 MB page via `CreateNewPage` — and copies only the live objects' vertex/index ranges into it with `CopyBufferRegion`, dropping the holes left by deleted geometry, then publishes the clone and retires the old page. That is the one that "creates a temporary allocation by copying only the valid buffers"; the command compaction above never does.
...
*Deferred from this slice, in rough build order:* one `ExecuteIndirect` per Viewport across pages (needs the command to carry per-command buffer views, or a `SceneEpoch` page directory the compute writes VBV/IBV from); the `SceneEpoch` atomic directory and revision-keyed cull cache (this slice regenerates every frame, so a static Viewport re-culls needlessly); retiring the persistent per-page indirect buffers; and a fence-gated read-back of `visibleCount` for a per-frame drawn-command telemetry counter.
